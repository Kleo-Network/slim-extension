...
export const DummyYTCards: PendingCard[] = [
	...
	// 3 Links
	{
		cardType: "DataCard",
		category: "Media & Communication",
		content: "Watched Daily Dose of Internet videos.Watched Daily Dose of Internet videos.Watched Daily Dose of Internet videos.",
		date: 1723398081,
...
export default function App({ user, setUser, slug }: UserDataProps) {
	...
	const [activeCard, setActiveCard] = useState<PendingCard>(
		activeCardList[0]
	);
				...
				fetchPendingCardData(replaceSlugInURL(GET_CARD_DETAIL, slug), {
					onSuccessfulFetch(data) {
						if (data) {
							...
							data = updateCardTypeToRenderInAllCards(
								data
							) as PendingCard[];
							setCards(data);
	...
	const removeCard = (id: string, hasToPublished: boolean) => {
		...
		managePendingCardCreation(
			replaceSlugInURL(CREATE_PUBLISHED_CARDS, slug),
			{
				method: "POST",
				headers: {
						...
						<div className="flex flex-col md:flex-row justify-between items-center mb-[25px]">
							<div className="flex flex-col md:flex-row justify-center items-stretch gap-4 mx-auto">
								...
								{activeCard.cardType == "DataCard" && (
									<div
										className={`rounded-lg shadow-lg p-3 px-5 flex flex-col justify-between min-h-[desiredMinHeight] max-h-[230px] mt-3 gap-2 ${
