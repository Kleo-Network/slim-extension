  ...
  return (
    ...
    >
      ...
      <header className="relative flex items-center backdrop-blur-sm h-[46px]">
        ...
        {Array.from(
          new Set(card.urls.map((url) => `https://www.google.com/s2/favicons?domain=${parseUrl(url.url)}&sz=40`)),
        ).map((iconUrl, index) => (
          <div key={iconUrl} className="w-6 h-6 flex-none rounded-full border-spacing-4 flex items-center">
            <img
        ...
        <div className="flex flex-row ml-auto items-center backdrop-blur-sm">
          <div className="flex font-inter text-sm text-white font-normal">{getDaysAgo(card.date)}</div>
        </div>
      </header>
